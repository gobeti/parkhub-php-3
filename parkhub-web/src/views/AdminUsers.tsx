...
export function AdminUsersPage() {
  ...
  async function saveQuota(userId: string) {
    const quotaNum = Number(editQuota);
    if (isNaN(quotaNum) || quotaNum < 0 || quotaNum > 999) {
      toast.error('Quota must be between 0 and 999');
      return;
    }
    setSavingQuota(true);
    try {
      const res = await api.adminUpdateUserQuota(userId, quotaNum);
      if (res.success && res.data) {
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, credits_monthly_quota: quotaNum } : u));
                    ...
                    {editingQuotaId === user.id ? (
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          onKeyDown={e => { if (e.key === 'Enter') saveQuota(user.id); if (e.key === 'Escape') setEditingQuotaId(null); }}
                        ...
                        <button onClick={() => saveQuota(user.id)} disabled={savingQuota} className="p-1 rounded hover:bg-emerald-100 dark:hover:bg-emerald-900/30 text-emerald-600">
                          {savingQuota ? <SpinnerGap weight="bold" className="w-3.5 h-3.5 animate-spin" /> : <Check weight="bold" className="w-3.5 h-3.5" />}
                        </button>
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-sm text-surface-600 dark:text-surface-300 hover:text-primary-600 dark:hover:text-primary-400 transition-colors group"
                        title="Click to edit monthly quota"
                      >
                        <Timer weight="bold" className="w-3.5 h-3.5 text-surface-400 group-hover:text-primary-500" />
                        {user.credits_monthly_quota}
                        <PencilSimple weight="bold" className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity text-surface-400" />
                      </button>
                    )}
                  </td>
                  <td className="px-5 py-4">
